...
ToolResult* tool_result_create_success(const char* tool_call_id, const char* format, ...) {
    tool_result_builder_t* builder = tool_result_builder_create(tool_call_id);
    if (builder == NULL) {
        return NULL;
    }
    ...
    if (tool_result_builder_set_success_json(builder, message) != 0) {
        free(message);
        tool_result_builder_destroy(builder);
        return NULL;
    }
    ...
    free(message);
    return tool_result_builder_finalize(builder);
}
...
ToolResult* tool_result_create_error(const char* tool_call_id, const char* format, ...) {
    ...
    if (tool_result_builder_set_error_json(builder, message) != 0) {
        free(message);
        tool_result_builder_destroy(builder);
        return NULL;
    }
