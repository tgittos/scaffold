...
- **`llm_client.c/h`** - LLM HTTP client wrapping http_client for API calls; accepts pre-built `const char** headers` from providers. Supports pluggable credential provider (`llm_credential_provider_fn`) for token refresh
...
- **`api_common.c/h`** - JSON payload building with prompt caching support (`SystemPromptParts` split, Anthropic `cache_control` markers, model-aware payload construction), `summarize_tool_calls()` for converting tool_calls JSON to plain text, `streaming_add_params()` shared utility for adding stream/store params
...
- **`tools_system.c/h`** - Core tool registry and execution framework, `generate_tools_json_flat()` for Responses API flat tool format
...
- **`openai_oauth_provider.c/h`** - OpenAI OAuth2 provider ops vtable: auth URL construction (PKCE + `codex_cli_simplified_flow`), code exchange, unified refresh token callback with rotation
...
- **`test_openai_oauth_provider.c`** - OpenAI OAuth provider tests (provider name, auth URL, unified vtable structure)
