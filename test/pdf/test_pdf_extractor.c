...
void test_pdf_extract_text_nonexistent_file(void) {
    ...
    #ifdef HAVE_MUPDF
    // With MuPDF, we should get an error about file not found
    TEST_ASSERT_NOT_NULL(result->error);
    TEST_ASSERT_NULL(result->text);
    TEST_ASSERT_EQUAL(0, result->page_count);
    #else
    // Without MuPDF, we should get a "not compiled" error
    TEST_ASSERT_NOT_NULL(result->error);
    TEST_ASSERT_TRUE(strstr(result->error, "MuPDF support not compiled") != NULL);
    #endif
